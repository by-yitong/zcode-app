...
class MarkdownBuilder implements md.NodeVisitor {
  ...
  List<Widget> build(List<md.Node> nodes) {
    ...
    // PATCH(zcode): flutter_markdown 已归档停更。特定 markdown 内容会让解析器
    // 留下未闭合的内联/表格状态, 原实现直接 assert 使整条消息渲染崩溃
    // (debug 红屏)。改为把残留内容冲刷进当前块后继续: 最多丢一层内联样式,
    // 不再崩溃。
    if (_tables.isNotEmpty) {
      _tables.clear();
    }
    if (_inlines.isNotEmpty) {
      for (final _InlineElement inline in _inlines) {
        _blocks.single.children.addAll(inline.children);
      }
      _inlines.clear();
    }
    _isInBlockquote = false;
